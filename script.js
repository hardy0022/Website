/* ══════════════════════════════════════════════════════════
   script.js — Canvas Frame Autoplay + Animations
...
(function () {
  ...
  const ANIMATION_DURATION_MS = 3200; // how long the full animation plays (ms)
  ...
  let heroActive     = true;
  let isReady        = false;
  let animPlaying    = false; // true while autoplay is running
  let animDone       = false; // true once animation has fully played
  let animStartTime  = null;  // RAF timestamp when autoplay began
  ...
  /* ─────────────────────────────────────────────────────────
     EASING
  ───────────────────────────────────────────────────────── */
  // ease-in-out cubic for silky smooth playback
  ...
  function playAnimation (timestamp) {
    ...
    // Fade hero text out during animation
    ...
    if (rawT < 1) {
      ...
      // Animation finished — scroll to next section
      animPlaying = false;
      animDone    = true;
      rafId       = null;
      scrollToMain();
  ...
  function triggerAnimation () {
    ...
    // Cancel any existing RAF and start fresh
  ...
  /* ─────────────────────────────────────────────────────────
     SCROLL / WHEEL / TOUCH — trigger animation on first interaction
  ───────────────────────────────────────────────────────── */
  function onFirstScroll (e) {
    if (animDone || animPlaying || !isReady) return;

    // Only trigger if user is at the top (on the hero)
  ...
  function onTouchMove (e) {
    ...
    if (deltaY > 15) { // swipe up = scroll down intent
      e.preventDefault();
      triggerAnimation();
    }
  }
  ...
  async function init () {
    ...
    // Listen for first scroll/wheel/touch to trigger animation
    window.addEventListener('wheel',      onFirstScroll, { passive: false });
    window.addEventListener('touchstart', onTouchStart,  { passive: true  });
    window.addEventListener('touchmove',  onTouchMove,   { passive: false });

    // Also support keyboard (arrow down / space / page down)
    window.addEventListener('keydown', (e) => {
      const keys = ['ArrowDown', 'Space', 'PageDown'];
      if (keys.includes(e.code) && !animDone && !animPlaying && window.scrollY <= 10) {
        e.preventDefault();
        triggerAnimation();
      }
    });
