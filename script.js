...
(function () {
  ...
  const LERP_FACTOR     = 0.055; // smoothing (lower = smoother/slower)
  ...
  /* ─────────────────────────────────────────────────────────
     ANIMATION LOOP
  ───────────────────────────────────────────────────────── */
  // Eased lerp — ease-out quad so fast scrolls decelerate smoothly
  ...
  function tick () {
    ...
    currentFrameF = lerpEased(currentFrameF, targetFrameF, LERP_FACTOR);
    currentFrameF = Math.min(TOTAL_FRAMES - 1, Math.max(0, currentFrameF));

    // Blend toward nearest frame for sub-frame smoothness
    const frameFloor = Math.floor(currentFrameF);
    const frameCeil  = Math.min(TOTAL_FRAMES - 1, frameFloor + 1);
    const frac       = currentFrameF - frameFloor;
    const frameIndex = frac >= 0.5 ? frameCeil : frameFloor;
